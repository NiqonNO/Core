...
namespace NiqonNO.Core.Editor
{
    ...
    public static class NOEditorSceneHandler
    {
        ...
        {
            List<string> sortedScenesToOpen = null;
            for (int i = 0; i < SceneManager.sceneCount; i++)
            {
                sortedScenesToOpen = sortedScenesToOpen == null ? 
                    NOSceneDependencyData.GetSceneDependencies(SceneManager.GetSceneAt(i).name, false) : 
                    NOSceneDependencyData.GetSceneDependencies(SceneManager.GetSceneAt(i).name, sortedScenesToOpen, false);
            }
